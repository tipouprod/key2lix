...
1. ادخل إلى **لوحة الأدمن** → **الإعدادات** → قسم **وضع الصيانة ونسخ احتياطي**.
2. اضغط **«إنشاء نسخة احتياطية الآن»** — سيتم إنشاء ملف `.db` وسيُحمّل تلقائياً إلى جهازك.
3. يمكنك تحميل أي نسخة سابقة من القائمة نفسها (رابط تحميل بجانب كل نسخة).

النسخ تُحفظ في `client/data/backup/`. إن ضبطت إعداد رفع السحابة (S3)، تُرفع النسخة تلقائياً بعد الإنشاء.

---

## نسخ احتياطي يدوي (سكربت)
