...
1. **زيادة الذاكرة في Railway**  
   **Settings** للخدمة → **Resources** (أو **Deploy** / **Variables**) → زِد **Memory** إن كان متاحاً في خطتك (مثلاً 512 MB → 1 GB). في الخطط المجانية قد لا يتوفر خيار الذاكرة؛ عندها انتقل لخطة مدفوعة (مثلاً Developer). ثم **Redeploy**.

2. **حد أقصى لذاكرة Node (اختياري)**  
   في **Variables** أضف:
   ```
   NODE_OPTIONS=--max-old-space-size=384
   ```
   (384 = ميجابايت). يحدّ ذلك استهلاك Node فلا يلتهم كل ذاكرة الحاوية ويُقتل. إن كانت ذاكرة الخدمة 512 MB فاستخدم 384 أو 400. إن زدت الذاكرة إلى 1 GB يمكن استخدام 768.

3. **التطبيق** يؤجل تحميل مكتبات ثقيلة (PDF، Excel، bcrypt، multer، speakeasy، QRCode) حتى أول طلب يحتاجها، لتقليل الذاكرة عند البدء. إن استمرت المشكلة زِد ذاكرة الخدمة أو انتقل إلى خطة أعلى.
