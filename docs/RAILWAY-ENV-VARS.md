...
1. **زيادة الذاكرة في Railway**  
   **Settings** للخدمة → **Resources** (أو **Deploy**) → زِد **Memory** إن كان متاحاً في خطتك (مثلاً 512 MB → 1 GB). ثم **Redeploy**.

2. **لا تضبط NODE_OPTIONS=--max-old-space-size** إلا إن كنت تريد تحديد حد أقصى لـ Node (مثلاً 384) لتجنب استهلاك كل ذاكرة الحاوية؛ ذلك قد يقلل احتمال قتل العملية من قبل النظام لكن التطبيق قد يصبح أبطأ عند الذروة.

3. **التطبيق** يحمّل مكتبات ثقيلة (PDF، Excel) عند أول طلب يحتاجها وليس عند التشغيل، لتقليل استهلاك الذاكرة عند البدء. إن استمرت المشكلة زِد ذاكرة الخدمة أو انتقل إلى خطة أعلى.
