...
### هل المشكلة من Variables؟

قد تساهم المتغيرات في المشكلة في حالات مثل:

| ماذا | التأثير |
|------|---------|
| **عدم وجود NODE_OPTIONS** | يزيد احتمال نفاد الذاكرة (OOM). أضف `NODE_OPTIONS=--max-old-space-size=384` في Variables. |
| **وجود PORT** | احذفه؛ اترك Railway يضبط المنفذ تلقائياً وإلا قد لا يفتح الموقع. |
| **مسافة حول =** | مثال خاطئ: `SMTP_HOST ="mail.example.com"` — قد لا يُقرأ المتغير. الصحيح: `SMTP_HOST=mail.example.com` (بدون مسافة قبل أو بعد =). |
| **عدم وجود BASE_URL** | غير حرج إن وُجد SITE_URL؛ يُفضّل إضافته بنفس قيمة SITE_URL للبريد والروابط. |

تأكد أن **SESSION_SECRET** غير فارغ وطوله 32+ حرفاً، وأن **NODE_ENV=production**.
