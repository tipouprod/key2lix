...
### Health Check على Railway

إذا ظهر "Application failed to respond" أو طلبات تُرجع **499** بعد وقت طويل:

1. **لا تضبط PORT** في Variables (اترك Railway يضبطه تلقائياً).
2. في **Settings** للخدمة: اضبط **Health Check**:
   - **Path:** `/ping` (مسار خفيف يرد فوراً بدون جلسة أو قاعدة بيانات).
   - **Timeout:** مثلاً 10 ثوانٍ.
3. أعد النشر (Redeploy). بعدها الموازن يعتبر الخدمة حية عند استجابة `/ping`، وطلبات الصفحة الرئيسية `/` قد تحتاج أول مرة لتحميل الجلسة وقاعدة البيانات.
