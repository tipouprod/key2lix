...
If you did not create an account, you can safely ignore this email.`;
  ...
  if (_emailJsServiceId && _emailJsTemplateId && _emailJsPublicKey && _emailJsPrivateKey) {
    return sendViaEmailJs({ to_email: to, verify_code: codeStr, subject: t.subject });
  }
  if (process.env.NODE_ENV !== 'production') {
    console.warn('Email verification skipped: لا يوجد SMTP ولا EmailJS مُعدّ. ضع SMTP_* أو EMAILJS_* في .env أو من لوحة الأدمن.');
  }
  return Promise.resolve(false);
}
