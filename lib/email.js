...
function sendMailWithDiagnostic(to, subject, text, html) {
  ...
  if (!transporter) {
    var hasEmailJs = !!(_emailJsServiceId && _emailJsTemplateId && _emailJsPublicKey && _emailJsPrivateKey);
    return Promise.resolve({
      success: false,
      error: 'لا يوجد SMTP مُعدّ (الاتصال المباشر غير متاح)',
      hint: hasEmailJs
        ? 'الرسالة التجريبية تُرسل عبر SMTP فقط. للإرسال الفعلي (مثل رمز التحقق) سيُستخدم EmailJS. لاختبار الإرسال استخدم قالب في EmailJS أو أضف إعدادات SMTP.'
        : 'املأ SMTP Host و User و Password وحفظ، أو استخدم EmailJS (الحقول الأربعة) ثم احفظ.'
    });
  }
  var bodyHtml = html || (text ? text.replace(/\n/g, '<br>') : '');
  if (bodyHtml) bodyHtml = wrapEmailWithLogo(bodyHtml);
