...
function initFromDb(db) {
  ...
  if (!transporter && !(_emailJsServiceId && _emailJsTemplateId && _emailJsPublicKey && _emailJsPrivateKey)) {
    ...
    if (missing.length) {
      var envHint = [];
      if (missing.indexOf('SMTP_HOST') !== -1) envHint.push('process.env.SMTP_HOST=' + (process.env.SMTP_HOST ? 'set' : 'NOT SET'));
      if (missing.indexOf('SMTP_USER') !== -1) envHint.push('process.env.SMTP_USER=' + (process.env.SMTP_USER ? 'set' : 'NOT SET'));
      if (missing.indexOf('SMTP_PASS') !== -1) envHint.push('process.env.SMTP_PASS=' + (process.env.SMTP_PASS ? 'set' : 'NOT SET'));
      console.warn('Email: غير مُعدّ — ناقص:', missing.join(', '), '|', envHint.join(', '));
    }
  } else if (transporter) {
...
function sendViaEmailJs(templateParams) {
  ...
  params.logo_url = params.logo_url || (getSiteUrl() + '/assets/img/logo6.png');
  params.site_name = params.site_name || 'Key2lix';
  ...
  if (params.body_html != null) {
    params.message = params.body_html;
    var maxLen = 45000;
    if (params.body_html.length > maxLen) {
      params.body_html = params.body_html.substring(0, maxLen) + '<p style="color:#94a3b8;font-size:12px;">… [محتوى مقصوص]</p>';
      params.message = params.body_html;
    }
  }
  var payload = {
...
function sendMail(to, subject, text, html) {
  ...
  if (!toAddr || !subject) return Promise.resolve(false);
  var bodyHtml = html || (text ? text.replace(/\n/g, '<br>') : '');
  var emailJsOk = !!(_emailJsServiceId && _emailJsTemplateId && _emailJsPublicKey && _emailJsPrivateKey);
  if (emailJsOk) {
    return sendViaEmailJs({ to_email: toAddr, subject: subject, body_html: bodyHtml || '' }).then(function (ok) {
      return !!ok;
    });
  }
  if (!transporter) {
    if (toAddr && subject) console.warn('Email: skipped (no SMTP). Would send to:', toAddr);
...
/** نفس sendMail لكن يُرجع سبب الفشل وتلميح الحل (للوحة الأدمن — إرسال تجريبي). */
function sendMailWithDiagnostic(to, subject, text, html) {
  var toAddr = normalizeEmail(to);
  if (!toAddr || !subject) {
    return Promise.resolve({ success: false, error: 'عنوان المستلم أو الموضوع فارغ', hint: 'أدخل بريداً صحيحاً في حقل «إرسال تجريبي إلى».' });
  }
  var bodyHtml = html || (text ? text.replace(/\n/g, '<br>') : '');
  var emailJsOk = !!(_emailJsServiceId && _emailJsTemplateId && _emailJsPublicKey && _emailJsPrivateKey);
  if (emailJsOk) {
    return sendViaEmailJs({ to_email: toAddr, subject: subject, body_html: bodyHtml || '' })
      .then(function (ok) {
        return ok ? { success: true } : { success: false, error: 'EmailJS رفض أو لم يرد', hint: 'تحقق من Template ID و Allow API requests في لوحة EmailJS.' };
      })
      .catch(function (err) {
        var msg = (err && err.message) || String(err);
        return { success: false, error: msg, hint: 'تحقق من Service ID و Template ID و Private Key و Allow API requests في EmailJS.' };
      });
  }
  if (!transporter) {
    return Promise.resolve({
      success: false,
      error: 'لا يوجد SMTP ولا EmailJS مُعدّ',
      hint: 'املأ SMTP Host و User و Password وحفظ، أو استخدم EmailJS (الحقول الأربعة) ثم احفظ.'
    });
  }
  if (bodyHtml) bodyHtml = wrapEmailWithLogo(bodyHtml);
  return transporter.sendMail({
    ...
    var msg = (err && err.message) || String(err);
    if (code === 'EAUTH' || (msg && msg.toLowerCase().includes('auth'))) {
      hint = 'تحقق من اسم المستخدم وكلمة المرور. لـ Gmail استخدم كلمة مرور التطبيق (App Password) من حساب Google → الأمان. لـ PrivateEmail استخدم كلمة مرور صندوق البريد.';
    } else if (code === 'ETIMEDOUT' || code === 'ESOCKET' || (msg && (msg.toLowerCase().includes('timeout') || msg.toLowerCase().includes('connection')))) {
      hint = 'الاتصال بالخادم فشل أو انتهت المهلة (مثلاً من Railway قد تُحجب منافذ SMTP). الحل: استخدم EmailJS — املأ الحقول الأربعة (Service ID, Template ID, Public Key, Private Key) واحفظ، ثم فعّل «Allow API requests» في لوحة EmailJS.';
    } else if (code === 'ECONNREFUSED') {
      hint = 'الخادم رفض الاتصال. تحقق من SMTP_HOST و SMTP_PORT (مثلاً mail.privateemail.com و 465 مع تفعيل SSL).';
    } else {
      hint = 'راجع إعدادات SMTP (Host, Port, User, Password) أو انتقل إلى EmailJS إذا كان الاستضافة يحجب منافذ البريد.';
    }
    console.warn('Email send failed:', msg, code, hint);
...
/** بناء HTML احترافي لرسالة التحقق (رمز التأكيد) — ألوان الموقع: #7c3aed, #5b21b6, #a78bfa, #22d3ee */
function buildVerificationEmailHtml(codeStr) {
  var code = String(codeStr || '').trim();
  return '<div style="font-family:\'Segoe UI\',Tahoma,Geneva,Verdana,sans-serif;color:#1e293b;font-size:15px;line-height:1.7;direction:rtl;text-align:right;max-width:520px;">' +
    '<p style="margin:0 0 8px;font-size:11px;color:#7c3aed;font-weight:600;text-transform:uppercase;letter-spacing:0.12em;">تأكيد البريد الإلكتروني</p>' +
    '<p style="margin:0 0 28px;font-size:22px;font-weight:700;color:#0f172a;letter-spacing:-0.02em;">مرحباً،</p>' +
    '<p style="margin:0 0 28px;color:#475569;font-size:16px;">شكراً لتسجيلك في Key2lix. لتفعيل حسابك، أدخل الرمز التالي في صفحة «حسابي»:</p>' +
    '<table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin:0 0 28px;width:100%;"><tr><td style="background:linear-gradient(135deg,#f5f3ff 0%,#ede9fe 100%);border:2px solid #7c3aed;border-radius:14px;padding:28px 24px;text-align:center;">' +
    '<p style="margin:0 0 8px;font-size:11px;color:#5b21b6;font-weight:600;text-transform:uppercase;letter-spacing:0.15em;">Verification Code</p>' +
    '<p style="margin:0;font-size:32px;font-weight:800;letter-spacing:10px;color:#5b21b6;font-family:\'Consolas\',\'Monaco\',monospace;">' + code + '</p>' +
    '<p style="margin:12px 0 0;font-size:12px;color:#6d28d9;">صالح 15 دقيقة</p>' +
    '</td></tr></table>' +
    '<table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin:0 0 28px;"><tr><td style="background:#faf5ff;border-radius:10px;padding:16px 20px;border-right:4px solid #7c3aed;">' +
    '<p style="margin:0 0 6px;font-size:14px;color:#475569;">• الرمز صالح لمدة 15 دقيقة.</p>' +
    '<p style="margin:0;font-size:14px;color:#475569;">• أدخله في صفحة «حسابي» في الموقع.</p></td></tr></table>' +
    '<p style="margin:0 0 32px;font-size:13px;color:#94a3b8;">إذا لم تقم بالتسجيل، يمكنك تجاهل هذا البريد بأمان.</p>' +
    '<div style="border-top:2px solid #ede9fe;padding-top:24px;margin-top:24px;direction:ltr;text-align:left;background:#faf5ff;border-radius:10px;padding:20px;border:1px solid #e9d5ff;">' +
    '<p style="margin:0 0 6px;font-size:11px;color:#6d28d9;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;">English</p>' +
    '<p style="margin:0 0 12px;font-size:15px;color:#334155;">Thank you for signing up. Use this code to verify your email:</p>' +
    '<p style="margin:0 0 8px;font-size:26px;font-weight:700;letter-spacing:6px;color:#5b21b6;font-family:\'Consolas\',monospace;">' + code + '</p>' +
    '<p style="margin:0;font-size:12px;color:#64748b;">Valid 15 min. Enter in "My account". If you didn\'t sign up, ignore this email.</p></div></div>';
}

/** التحقق من البريد الإلكتروني — إرسال الرقم السري (OTP). يُرجع Promise<boolean>. نفضّل EmailJS إن وُجد ثم SMTP. */
...
If you did not create an account, you can safely ignore this email.`;
  ...
  if (_emailJsServiceId && _emailJsTemplateId && _emailJsPublicKey && _emailJsPrivateKey) {
    var verificationBody = buildVerificationEmailHtml(codeStr);
    return sendViaEmailJs({ to_email: to, subject: t.subject, body_html: verificationBody, verify_code: codeStr }).then(function (ok) {
      if (!ok) console.warn('Verification email: EmailJS إرسال فشل. تحقق من لوحة EmailJS و Allow API requests.');
      return ok;
    });
  }
  if (transporter) {
    return sendMail(to, t.subject, t.body, buildVerificationEmailHtml(codeStr)).then(function (ok) {
...
/** بناء HTML احترافي لرسالة إعادة تعيين كلمة المرور — ألوان الموقع. */
function buildPasswordResetEmailHtml(link) {
  var url = String(link || '').trim();
  return '<div style="font-family:\'Segoe UI\',Tahoma,Geneva,Verdana,sans-serif;color:#1e293b;font-size:15px;line-height:1.7;direction:rtl;text-align:right;max-width:520px;">' +
    '<p style="margin:0 0 8px;font-size:11px;color:#7c3aed;font-weight:600;text-transform:uppercase;letter-spacing:0.12em;">إعادة تعيين كلمة المرور</p>' +
    '<p style="margin:0 0 28px;font-size:22px;font-weight:700;color:#0f172a;letter-spacing:-0.02em;">مرحباً،</p>' +
    '<p style="margin:0 0 24px;color:#475569;font-size:16px;">تلقينا طلباً لإعادة تعيين كلمة المرور. لتعيين كلمة مرور جديدة، اضغط على الزر أدناه (الرابط صالح لمدة ساعة):</p>' +
    '<table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin:0 0 28px;"><tr><td style="border-radius:12px;padding:0;text-align:center;">' +
    '<a href="' + url + '" style="display:inline-block;background:linear-gradient(135deg,#7c3aed 0%,#6d28d9 100%);color:#ffffff !important;font-weight:600;font-size:15px;text-decoration:none;padding:16px 28px;border-radius:10px;">إعادة تعيين كلمة المرور</a>' +
    '</td></tr></table>' +
    '<table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin:0 0 24px;width:100%;"><tr><td style="background:#faf5ff;border:1px solid #e9d5ff;border-radius:10px;padding:14px 18px;border-right:4px solid #7c3aed;">' +
    '<p style="margin:0;font-size:12px;color:#64748b;word-break:break-all;">أو انسخ الرابط: ' + url + '</p></td></tr></table>' +
    '<p style="margin:0 0 32px;font-size:13px;color:#94a3b8;">إذا لم تطلب إعادة التعيين، تجاهل هذا البريد؛ حسابك آمن.</p>' +
    '<div style="border-top:2px solid #ede9fe;padding-top:24px;margin-top:24px;direction:ltr;text-align:left;background:#faf5ff;border-radius:10px;padding:20px;border:1px solid #e9d5ff;">' +
    '<p style="margin:0 0 6px;font-size:11px;color:#6d28d9;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;">English</p>' +
    '<p style="margin:0 0 16px;font-size:15px;color:#334155;">Password reset requested. Click the button below (valid 1 hour):</p>' +
    '<p style="margin:0 0 12px;"><a href="' + url + '" style="display:inline-block;background:linear-gradient(135deg,#7c3aed 0%,#6d28d9 100%);color:#ffffff !important;font-weight:600;font-size:14px;text-decoration:none;padding:14px 24px;border-radius:8px;">Reset Password</a></p>' +
    '<p style="margin:0;font-size:12px;color:#64748b;">If you didn\'t request this, ignore this email.</p></div></div>';
}
...
If you did not request this, you can ignore this email; your account is secure and your password will not be changed.`;
  ...
  if (_emailJsServiceId && _emailJsTemplateId && _emailJsPublicKey && _emailJsPrivateKey) {
    return sendViaEmailJs({ to_email: to, subject: t.subject, body_html: buildPasswordResetEmailHtml(resetLink), reset_link: resetLink });
  }
  if (transporter) return sendMail(to, t.subject, t.body, buildPasswordResetEmailHtml(resetLink));
  return Promise.resolve(false);
}
