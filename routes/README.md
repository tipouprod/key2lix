...
| **vendor-api.js** | `/api/vendor/*` — تسجيل، دخول، 2FA، ملف شخصي، مفاتيح API، webhook، منتجات، طلبات، تقارير، استيراد كتالوج، تسوية PDF. **ملاحظة:** حالياً مسارات المورد مُعرّفة داخل `server.js` ولا يُستدعى `registerVendorApi`؛ الملف جاهز للاستخدام عند التكامل مستقبلاً. | `registerVendorApi(app, opts)` |
